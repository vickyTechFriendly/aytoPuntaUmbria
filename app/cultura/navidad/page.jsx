...
export default function Navidad() {
  const { setTitle, setDescription, setKeywords } = usePageContext();
    ...
    useEffect(() => {
        setTitle('Navidad - Ayuntamiento de Punta Umbría');
        setDescription('Web oficial del Excmo. Ayuntamiento');
        setKeywords('Punta Umbría, Navidad, Fiestas y Zambombas');
    }, []);
      ...
      .then(data => {
        if (Array.isArray(data.data)) {
          const noticiasFiltradasYOrdenadas = ordenarNoticiasPorFecha(data.data)
            .filter(noticia => noticia.categorias.includes('Navidad'));
    ...
    return (
      <main>
        <section  className={styles.cabecera}>
          <img src="https://images.unsplash.com/photo-1512389142860-9c449e58a543?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8Y2hyaXN0bWFzfGVufDB8fDB8fHww" alt="" />
          <h1>Navidad</h1>
        </section>
