...
export default function Eventos() {
  const { setTitle, setDescription, setKeywords } = usePageContext();
    ...
    useEffect(() => {
        ...
        setKeywords('Punta Umbría, agenda, eventos');
    }, []);
    ...
    fetch(`${process.env.NEXT_PUBLIC_AGENDA_URL}`)
      ...
      .then(data => {
        if (Array.isArray(data.data)) {
          ...
          setNoticias([]); // Asegurarse de que noticias sea un array, incluso si está vacío
        }
        setIsLoading(false);
      })
      .catch(error => {
        ...
        setIsLoading(false);
        setNoticias([]); // En caso de error, también asegurarse de que noticias sea un array vacío
      });
  }, []);
    ...
    return (
      <main>
        <section className={styles.agenda}>
          ...
          <h1 tabIndex="0" id="contenido">Agenda</h1>
          <div className={styles.categorias}>
            <p className={styles.sugerencia}>Busca un evento por la categoría y fecha que más te interese</p>
        ...
        <section className={`${styles.listadoNoticias} ${styles.listadoEventos}`}>
          ...
          <Link href={`/agenda/${noticia.slug}`} key={index} className={styles.noticia} id="contenido">
            <img src={`${process.env.NEXT_PUBLIC_IMAGENES_URL}/${noticia.imagen1}`}  alt=""></img>
